...
static void getter(char *path) {
	sprintf((char *)g_sendBuffer, MAIN_PREFIX_BUFFER_GET, path);
	send(tcp_client_socket, g_sendBuffer, strlen((char *)g_sendBuffer), 0);
}

static void poster(char *path, char status){
	char *contentBuffer[5];
	sprintf(contentBuffer, "LED=%c", status);
	sprintf((char *)g_sendBuffer, MAIN_PREFIX_BUFFER_POST, path, strlen(contentBuffer), contentBuffer);
	printf(g_sendBuffer);
	send(tcp_client_socket, g_sendBuffer, strlen((char *)g_sendBuffer), 0);
}
...
static void task_process(void *pvParameters) {
  ...
  while(1){
    ...
    switch(state){
	  ...
	  //    status = !status;
	  //    sprintf(post_content, "LED=%c", status);
	  //    sprintf((char *)g_sendBuffer, "POST / HTTP/1.0\nContent-Type: application/x-www-form-urlencoded\nContent-Length: %d\n\n%s", strlen(post_content), post_content);
	  //    send(tcp_client_socket, g_sendBuffer, strlen((char *)g_sendBuffer), 0);
      //}
      ...
      if(xQueueReceive(xQueueMsg, &p_recvMsg, 5000) == pdTRUE){
		...
		char *p_led = strstr(p_recvMsg->pu8Buffer, "led");
		status = *(p_led+7) - 48;
		
		if (status) pio_clear(LED_PIO, LED_IDX_MASK);
