...
#define BUT1_PIO_IDX		8
#define BUT1_PIO_MASK		(1 << BUT1_PIO_IDX)
...
void but_callback(void) {
	...
	//Queue
	int value = -100;
	xQueueSendFromISR(xQueueIncDec, &value, &xHigherPriorityTaskWoken);
}
...
int main(void) {
	...
	if (xSemaphoreBut == NULL) printf("Falha em criar o semáforo BUT \n");
		
	xSemaphoreBut1 = xSemaphoreCreateBinary();
	if (xSemaphoreBut1 == NULL) printf("Falha em criar o semáforo BUT1 \n");
	...
	if (xQueueLedFreq == NULL) printf("Falha em criar a queue \n");
